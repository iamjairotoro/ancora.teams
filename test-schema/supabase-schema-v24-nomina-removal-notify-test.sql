...
begin
  ...
  if removed_member_id is not null and exists (select 1 from services where id = affected_service_id) then
    -- VERSIÓN DE PRUEBA: comentado para que un UPDATE/DELETE en esta base de
    -- prueba no dispare una llamada HTTP real contra la app de producción.
    -- Se deja comentado (no borrado) para poder ver exactamente qué hacía
    -- el original.
    /*
